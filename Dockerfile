...
# Prepare 
ControlNet models folder (harmless if empty)
...
RUN cd /workspace/stable-diffusion-webui && \
    ...
    (pip 
install -r requirements_versions.txt || pip install -r requirements.txt) && \
    # Extension requirements (ignore if files move)
    pip install -r extensions/sd-webui-deforum/requirements.txt ||
true && \
...
true && \
    ...
    # Some revs of k-diffusion import `clip`;
...
RUN dos2unix /workspace/start.sh # <-- Added dos2unix command here
